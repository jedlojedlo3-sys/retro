...
const STORAGE_KEY = 'retro_boutique_custom_products_v2';
const STORAGE_DELETED_KEY = 'retro_boutique_deleted_ids_v2';
const MIGRATION_DONE_KEY = 'retro_boutique_v2_migrated';

function ensureMigration() {
  if (typeof window === 'undefined') return;
  try {
    if (!localStorage.getItem(MIGRATION_DONE_KEY)) {
      // Clear legacy demo storage
      localStorage.removeItem('retro_boutique_custom_products');
      localStorage.removeItem('retro_boutique_deleted_ids');
...
export function saveClientProduct(product: Product): void {
  ...
  try {
    ...
    // Remove from deleted IDs if it was there
    const deletedIds: string[] = JSON.parse(localStorage.getItem(STORAGE_DELETED_KEY) || '[]');
